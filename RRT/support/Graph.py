...
class Graph:
	def __init__(self,V,pos_node_start,E,Cobs,target,check_edge): # Vertices, Edges
		...
		for i in range(size(self.Cobs)): # checks every Obstacle for edge overlaps
			...
			if (node_attach.x <= x1 and node_new.x <= x1) or (node_attach.x >= x4 and node_new.x >= x4) or (node_attach.y <= y1 and node_new.y <= y1) or (node_attach.y >= y4 and node_new.y >= y4):
				...
				if xi >= x1 and xi <= x4 and yi >= y1 and yi <= y4 and dist_new < dist:
					node_new_in_Cfree = self.generate_node([xi,yi])
					dist = dist_new
	...
	def check_target(self):
		...
		V,sizeV,target = self.V,self.sizeV,self.target
		nodes_to_target = []
		index,cost = -1,-1
		for i in range(1,sizeV): # does not check node_start
			for j in range(size(target)):
				...
				if (and1 and and2) and (V[i].cost < cost or cost == -1):
					cost = V[i].cost # cumulative cost of the entire road
					index = i # i == V[i].index
		if index != -1:
			nodes_to_target.append(index)
			# print('index = ', index)
			while type(V[index].parent) != str: # case: V[index].parent = 'none' --> node_start reached
				index = V[index].parent.index
				nodes_to_target.append(index)
				# print('index = ', index)
			# print('\n')
		return nodes_to_target, round(cost,1)
	...
	def draw(self,screen,nodes_to_target):
		...
		# 	circle(screen, 'red', [ V[i].x, V[i].y ], 1)
			# screen.blit(font.render(str(i+1), True, 'red'), (V[i].x + 5, V[i].y - 5)) # numbers on the nodes
			# screen.blit(font.render(str(i+1) + ' ' + str(int(V[i].x)) + ',' + str(int(V[i].y)), True, 'red'), (V[i].x + 5, V[i].y - 5)) # numbers on the nodes + coordinates
	...
	# def check_target(self): # versione che avrebbe dovuto migliorare l'aggiornamento dei costi ma che in realta' sminchia tutto
	# 	"""#checks every node in target (if node.parent not in target) and goes backwards to node_start, then chooses the road with the smallest cumulative cost"""
	...
	# 			and2 = (V[i].parent.x <= target[j].x or V[i].parent.x >= target[j].x + target[j].lx) or (V[i].parent.y <= target[j].y or V[i].parent.y >= target[j].y + target[j].ly) # parent outside target
	# 			if (and1 and and2):
	# 				temp_nodes_to_target = []
	# 				temp_index = i
	# 				temp_nodes_to_target.append(temp_index)
	# 				while type(V[temp_index].parent) != str: # case: V[index].parent = 'none' --> node_start reached
	# 					temp_index = V[temp_index].parent.index
	# 					temp_nodes_to_target.append(temp_index)
	# 				for k in range(1,size(temp_nodes_to_target)): # updates costs in the path
	# 					self.V[temp_nodes_to_target[k]].cost = self.V[temp_nodes_to_target[k-1]].cost + distance_nodes(self.V[temp_nodes_to_target[k]], self.V[temp_nodes_to_target[k-1]])
	# 				if (self.V[i].cost < cost or cost == -1):
