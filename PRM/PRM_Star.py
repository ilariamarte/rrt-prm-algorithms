...
from support.imports_PRM_Star import *
...
def init_PRM_Star(Cobs,pos_node_start):
	V = [1]
	E = []
	...
	d, zeta_d = 2, pi
	gamma_PRM_Star = 2*power(1 + 1/d, 1/d) * power(vol_Cfree/zeta_d, 1/d)
	if isnan(gamma_PRM_Star):
		print("gamma_PRM_Star isnan: True")
	return G,gamma_PRM_Star,d,new


def PRM_Star(G,radius,new_group,new):
...
if __name__ == '__main__':
	...
	while loop:
		...
		if start_init == True:
			G,gamma_PRM_Star,d,new = init_PRM_Star(Cobs,pos_node_start)
			start_init = False
			start_algorithm = True
		
		if start_algorithm == True:
			new = 0
			radius = (gamma_PRM_Star+1) * power(log(new_group)/new_group, 1/d)
			new_group,new = PRM_Star(G, radius, new_group, new)
			G.draw(screen, new)
